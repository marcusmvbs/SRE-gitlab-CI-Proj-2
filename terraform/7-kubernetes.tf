resource "google_container_cluster" "primary" { 
  ...
  addons_config {      # Istio service mesh (next addon)
    http_load_balancing {
      disabled = true  # Disabled to use nginx as ingress controller
    }
    horizontal_pod_autoscaling {
      disabled = false # Autoscaling enabled
    }
  }

  release_channel {
    channel = "REGULAR" # it manages k8s cluster upgrade
  }
